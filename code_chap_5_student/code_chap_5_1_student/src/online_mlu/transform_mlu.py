...
def run_ori_power_diff_pb():
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            start_time = time.time()
            ret = sess.run(output_tensor,feed_dict={input_tensor_pow:2, input_tensor:[X]})
            end_time = time.time()
...
def run_numpy_pb():
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            input_tensor = sess.graph.get_tensor_by_name('X_content:0')
            input_tensor_pow = sess.graph.get_tensor_by_name('moments_15/PowerDifference:0')
            output_tensor = sess.graph.get_tensor_by_name('add_37:0')
            conv_tensor = sess.graph.get_tensor_by_name('Conv2D_13:0')
            grad_tensor = sess.graph.get_tensor_by_name('moments_15/StopGradient:0')

            start_time = time.time()
            input_x, input_y = sess.run([conv_tensor, grad_tensor], feed_dict={input_tensor: [X]})
            output = power_diff_numpy(input_x, input_y, 2).reshape(1, 256, 256, 3)
            ret = sess.run(output_tensor, feed_dict={input_tensor:[X], input_tensor_pow:output})
            end_time = time.time()
