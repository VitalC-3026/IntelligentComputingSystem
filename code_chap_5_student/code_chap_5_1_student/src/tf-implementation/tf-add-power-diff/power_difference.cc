...
Status MLUPowerDifference::CreateMLUOp(std::vector<MLUTensor *> &inputs,
  ...
  // 不能只考虑长度一样的情况，长度不同时需要广播
  lib::MLUTensorUtil util1(input1);
  lib::MLUTensorUtil util2(input2);
  lib::MLUTensorUtil utilo(output);
  int len3 = utilo.NumElements(); 

  int output_dims = utilo.dims();
  int len = 1;
  std::vector<int> output_shape(output_dims);
  for (int i = 0; i < output_dims; ++i) {
    output_shape[i] = utilo.dim_size(i);
    len *= output_shape[i];
  }
