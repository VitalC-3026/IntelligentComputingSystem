...
def net(image, type=0):
    ...
    convt1 = _conv_tranpose_layer(resid5, 64, 3, 2)
    convt2 = _conv_tranpose_layer(convt1, 32, 3, 2)
    convt3 = _conv_tranpose_layer(convt2, 3, 9, 1)

    #TODO：最后一个卷积层的输出再经过 tanh 函数处理，最后的输出张量 preds 像素值需限定在 [0,255] 范围内
    preds = tf.nn.tanh(convt3) * 150 + 255./2
    return preds
...
def _conv_layer(net, num_filters, filter_size, strides, relu=True, type=0):
    ...
    # TODO：进行卷积计算
    net = tf.nn.conv2d(net, filters=weights_init, strides=strides_shape, padding='SAME')
...
def _conv_tranpose_layer(net, num_filters, filter_size, strides, type=0):
    ...
    print(new_shape)
    tensor_shape = tf.stack(new_shape)
    print(tensor_shape)

    # TODO：进行转置卷积计算, 这里的data format是NHWC
    net = tf.nn.conv2d_transpose(net, weights_init, tensor_shape, stride_shape, padding="SAME")
...
def _residual_block(net, filter_size=3, type=0):
    ...
    # 写死了output_channels是128
    current = _conv_layer(net, 128, filter_size, 1, True, type)
    current = _conv_layer(current, 128, filter_size, 1, False, type)
    net = current + net
    return net

def _batch_norm(net, train=True):
    batch, rows, cols, channels = [i.value for i in net.get_shape()]
    axes=list(range(len(net.get_shape())-1))  # 得到均值和方差
