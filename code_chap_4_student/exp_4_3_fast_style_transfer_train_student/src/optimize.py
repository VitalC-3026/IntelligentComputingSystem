...
def loss_function(net, content_features, style_features, content_weight, style_weight, tv_weight, preds, batch_size):
    ...
    y_tv = 2 * tf.nn.l2_loss(tv_x_size - preds)
    x_tv = 2 * tf.nn.l2_loss(tv_y_size - preds)
...
def optimize(content_targets, style_target, content_weight, style_weight,
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'), tf.Session() as sess:
        ...
        style_image = tf.compat.v1.placeholder(tf.float32, shape=style_shape,
                                         name='style_image')

        #TODO: 依次调用 vgg.py 文件中的 preprocess()、net() 函数对风格图像进行预处理，并将此时得到的特征提取网络传递给 net
        style_pre = vgg.preprocess(style_image)
        ...
        X_content = tf.placeholder(tf.float32, shape=batch_shape, name='img_content')
            ...
            while iterations * batch_size < num_examples:
                ...
                if should_print:
                    ...
                    if slow:
                        ...
                        print(save_path)
                        print(save_path)
                        dir_path = save_path.split("/")[0]
                        print(dir_path)
                        if not os.path.exists(dir_path):
                            os.mkdir(dir_path)
                        tmp_path = save_path.split('.ckpt')
                        act_path = tmp_path[0] + str(iterations) + ".ckpt"
                        print(tmp_path)
                        print(act_path)
                        res = saver.save(sess, act_path)
                    # 将相关计算结果返回
