...
def net(data_path, input_image):
    ...
    for i, name in enumerate(layers):
        if name[:4] == 'conv':
            ...
            kernels = kernels.transpose(1, 0, 2, 3)  # 1,0,2,3
            print(kernels.shape)
            ...
            kernels, bias = weights[i][0][0][0][0]
            output = tf.reshape(current, [-1, current.shape[1]*current.shape[2]*current.shape[3]])
            kernels = tf.reshape(kernels, [kernels.shape[0]*kernels.shape[1]*kernels.shape[2], 4096])  # -1可以写成是4096 
            # print(output.shape)
            # print(kernels.shape)
            current = tf.nn.bias_add(tf.matmul(output, kernels), bias.flatten())
        elif name  == 'fc7':
            kernels, bias = weights[i][0][0][0][0]
            # print(kernels.shape)
            kernels = tf.reshape(kernels, [4096, 4096]) # -1可以写成是4096
            # print(kernels.shape)
            current = tf.nn.bias_add(tf.matmul(current, kernels), bias.flatten())
        elif name  == 'fc8':
            kernels, bias = weights[i][0][0][0][0]
            # print(kernels.shape)
            kernels = tf.reshape(kernels, [4096, 1000]) # -1可以写成是1000
            # print(kernels.shape)
            current = tf.nn.bias_add(tf.matmul(current, kernels), bias.flatten())
...
def _conv_layer(input, weights, bias):
    ...
    # 很重要的地方！bias.flatten() (1, 64) => (64, )
    output = tf.nn.bias_add(conv, bias.flatten())
    # output = tf.nn.relu(output)  # 这个要不要加呢？不用，nets中有专门的relu层
    return output

def _pool_layer(input):
    # TODO: 定义最大池化的操作步骤，input 为输入张量，返回池化操作后的计算结果
    output = tf.nn.max_pool2d(input, [1, 2, 2, 1], [1, 2, 2, 1], padding='VALID')  # 如果没有padding填成VALID，这里即使填成SAME，在fc6层没有写死512*7*7的话也不会报错。
    # 你竟然忘了return!!
    return output
