...
def ffwd(data_in, paths_out, model, device_t='', batch_size=1):
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            num_iters = int(len(paths_out)/batch_size)  # 一次可能好几张图同时进行预测
            for i in range(num_iters):
                pos = i * batch_size
                curr_batch_out = paths_out[pos:pos+batch_size]  # 注意这个pos的使用
                ...
                if is_paths:
                    ...
                    for j, p in enumerate(curr_batch_in):
                        X[j] = get_img(p, shape)
                else:
                ...
                # TODO：在该批次下调用 utils.py 中的 save_img() 函数对所有风格迁移后的图片进行存储
                    save_img(path_out, _preds[j]) # 这个填_preds
